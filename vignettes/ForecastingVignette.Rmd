...
plot(hetpreds$DOY, hetpreds$Mean, type = "l",
     ...
     col = "darkorange")
lines(hetpreds$DOY, sugpreds$Lower, lwd = 2, lty = 2, col = "darkorange")
lines(hetpreds$DOY, sugpreds$Upper, lwd = 2, lty = 2, col = "darkorange")
...
The empirical as well as hetGP climatological models both do reasonably well with respect to prediction and prediction interval coverage, although prediction intervals of both models fail to cover 6 observations near the end of the forecast period.  
...
```{r}
# function to generate MVN realizations (using svd)
rmultnorm <- function(n, mu, sigma){
  # returns a n x length(mu) matrix
  p <- length(mu)
...
We can then extract means and quantiles and make a plot.
```{r, fig.dim=c(8,6)}
